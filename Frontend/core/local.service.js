...
function _nuevoId(coleccion) {
  return coleccion.length > 0
    ? Math.max(...coleccion.map(x => x.id)) + 1
    : 1;
}
...
const userRoles = [
  ...
  const linkCodes = Array.from({ length: 10 }, (_, i) => {
    ...
    const prefijo = proposito === 'ENROLLMENT'
      ? 'ENR'
      : 'DOC';

    return {
      id: i + 1,
      code: `LUM-${prefijo}-2026-${(i + 1)
        .toString()
        .padStart(4, '0')}`,
      purpose: proposito,
      status: estado,
      issuedById: 1,
      expiresAt: estado === 'EXPIRED'
        ? _hoy(-5)
        : _hoy(30),
      usedById: estado === 'USED'
        ? 1
        : null,
      usedAt: estado === 'USED'
        ? _hoy(-2)
...
const TeacherService = {
  ...
  getAll: () =>
    _delay([..._leerDB().teachers]),

  getById: (id) =>
    _delay(
      _leerDB().teachers.find(
        t => t.id === parseInt(id)
      )
    ),
...
const StudentService = {
  ...
  getAll: () =>
    _delay([..._leerDB().students]),

  getById: (id) =>
    _delay(
      _leerDB().students.find(
        s => s.id === parseInt(id)
      )
    ),
  ...
  getByGroup: (groupId) =>
    _delay(
      _leerDB().students.filter(
        s => s.groupId === parseInt(groupId)
      )
    ),
...
const ClassGroupService = {
  ...
  getAll: () =>
    _delay([..._leerDB().classGroups]),

  getById: (id) =>
    _delay(
      _leerDB().classGroups.find(
        g => g.id === parseInt(id)
      )
    ),
  ...
  update(id, datos) {
    ...
    const idx = db.classGroups.findIndex(
      g => g.id === parseInt(id)
    );
...
const SubjectService = {
  ...
  getAll: () =>
    _delay([..._leerDB().subjects]),

  getById: (id) =>
    _delay(
      _leerDB().subjects.find(
        s => s.id === parseInt(id)
      )
    ),
  ...
  update(id, datos) {
    ...
    const idx = db.subjects.findIndex(
      s => s.id === parseInt(id)
    );
...
const GroupSubjectService = {
  ...
  getAll: () =>
    _delay([..._leerDB().groupSubjects]),

  getByGroup: (groupId) =>
    _delay(
      _leerDB().groupSubjects.filter(
        gs => gs.groupId === parseInt(groupId)
      )
    ),
  ...
  create(datos) {
    ...
    const idx = db.groupSubjects.findIndex(
      gs => gs.id === parseInt(id)
    );
...
const LinkCodeService = {
  ...
  getAll: () =>
    _delay([..._leerDB().linkCodes]),

  getByPurpose: (purpose) =>
    _delay(
      _leerDB().linkCodes.filter(
        c => c.purpose === purpose
      )
    ),
  ...
  revoke(id) {
    ...
    const idx = db.linkCodes.findIndex(
      c => c.id === parseInt(id)
    );
...
function reiniciarBaseLocal() {
  ...
  console.log(
    'Base local de LUMINA reiniciada. Recarga la página.'
  );
}